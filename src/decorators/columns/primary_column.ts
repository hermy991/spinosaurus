...
export function PrimaryColumn(options: ColumnOptions = {}): any {
  return  (entityf: Object, propertyKey: string, descriptor: PropertyDescriptor) => {
     ...
     */
    const entity = { target: entityf instanceof Function ? <Function>entityf : entityf.constructor, name: entityf.constructor.name };
    const property = { propertyKey, type: reflect_metadata.Reflect.getMetadata("design:type", entityf, propertyKey) };
    const target: ColumnOptions = { name: propertyKey, type: getColumnType({type: property.type}) };
