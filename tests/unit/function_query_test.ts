...
Deno.test(
  testMessage.replace(/\{\}/ig, "select [select distinct *] query"),
  () => {
    const db: Connection = new Connection(con1);
...
Deno.test(
  testMessage.replace(
    /\{\}/ig,
    "select [select * from 'Entity'] query",
  ),
  () => {
    const db: Connection = new Connection(con1);
...
Deno.test(
  testMessage.replace(/\{\}/ig, "select [select columns] query"),
  () => {
    const db: Connection = new Connection(con1);
...
Deno.test(
  testMessage.replace(
    /\{\}/ig,
    "select [select distinct columns] query",
  ),
  () => {
    const db: Connection = new Connection(con1);
...
Deno.test(
  testMessage.replace(/\{\}/ig, "select [select column as] query"),
  () => {
    const db: Connection = new Connection(con1);
...
Deno.test(
  testMessage.replace(/\{\}/ig, "select [where] query"),
  () => {
    const db: Connection = new Connection(con1);
    const qs = db.select([`u."userName"`], [`u."firstName"`])
      .from({ entity: "User", as: "u" })
...
Deno.test(
  testMessage.replace(/\{\}/ig, "select [where like] query"),
  () => {
    const db: Connection = new Connection(con1);
    const qs = db.select([`u."userName"`], [`u."firstName"`])
      .from({ entity: "User", as: "u" })
...
Deno.test(
  testMessage.replace(/\{\}/ig, "select [where not like] query"),
  () => {
    const db: Connection = new Connection(con1);
    const qs = db.select([`u."userName"`], [`u."firstName"`])
      .from({ entity: "User", as: "u" })
...
Deno.test(
  testMessage.replace(
    /\{\}/ig,
    "select [where between(string)] query",
  ),
  () => {
    const db: Connection = new Connection(con1);
    const qs = db.select([`u."userName"`], [`u."firstName"`])
...
Deno.test(
  testMessage.replace(
    /\{\}/ig,
    "select [where between(number)] query",
  ),
  () => {
    const db: Connection = new Connection(con1);
    const qs = db.select([`u."userName"`], [`u."firstName"`])
...
Deno.test(
  testMessage.replace(
    /\{\}/ig,
    "select [where between(Date)] query",
  ),
  () => {
    const db: Connection = new Connection(con1);
    const qs = db.select([`u."userName"`], [`u."firstName"`])
...
Deno.test(
  testMessage.replace(/\{\}/ig, "select [where with params] query"),
  () => {
    const db: Connection = new Connection(con1);
    const qs = db.select([`u."userName"`], [`u."firstName"`])
      .from({ entity: "User", as: "u" })
...
Deno.test(
  testMessage.replace(/\{\}/ig, "select [order by] query"),
  () => {
    const db: Connection = new Connection(con1);
    const qs = db.select([`u."userName"`], [`u."firstName"`])
      .from({ entity: "User", as: "u" })
...
Deno.test(
  testMessage.replace(
    /\{\}/ig,
    "select [order by with direction] query",
  ),
  () => {
    const db: Connection = new Connection(con1);
    const qs = db.select([`u."userName"`], [`u."firstName"`])
