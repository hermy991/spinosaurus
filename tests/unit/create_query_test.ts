...
Deno.test(testMessage.replace(/\{\}/ig, "create [create table] query should work"), () => {
  ...
  const querySpected = `CREATE TABLE "public"."User" ( "column1" VARCHAR )`.replace(/[ \n\t]+/ig, " ").trim();
...
Deno.test(testMessage.replace(/\{\}/ig, "create [create table with data] query should work"), () => {
  ...
  let query = qs.getQuery() || "";
  query = query.replaceAll(/[ \n\t]+/ig, " ").trim();
  const querySpected = `CREATE TABLE "public"."User" ( "column1" VARCHAR ); INSERT INTO "public"."User" ("column1") VALUES ('hola'); INSERT INTO "public"."User" ("column1") VALUES ('xx')`.replace(/[ \n\t]+/ig, " ").trim();
  assertEquals(query, querySpected);
});