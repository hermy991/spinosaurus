...
Deno.test(testMessage.replace(/\{\}/ig, "rename table execute() function should work"), async () => {
...
Deno.test(testMessage.replace(/\{\}/ig, "rename table column execute() function should work"), async () => {
  let db: Connection = new Connection(con1);
  let oldEntity = `RenameTable_${window.OBJECT_SEQUENCE++}`;
...
Deno.test(testMessage.replace(/\{\}/ig, "rename table and column execute() function should work"), async () => {
  let db: Connection = new Connection(con1);
  let oldEntity = `RenameTable_${window.OBJECT_SEQUENCE++}`;
  let newEntity = `RenameTable_${window.OBJECT_SEQUENCE++}`;
  let chk1 = await db.checkObject({ name: oldEntity });
